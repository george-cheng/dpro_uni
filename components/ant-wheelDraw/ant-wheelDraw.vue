...
<script>
	import wheelDraw from "./ant-wheelDraw.js"
	export default {
		data() {
			return {
		...
		props: {
			...
			prizeList:{
				// default:()=>{
					...
					// var text = [];
					// for(let i in this.dedAmount){
					// 	text.push('', this.dedAmount[i].name)
					// 	console.log(text)
					// }
					// return text
				// },
		...
		created(){
			let prizeListArr = []
				for(let i in this.prizeList){
					prizeListArr.push('', this.prizeList[i].name)
				}
				this.prizeList = prizeListArr
				console.log(typeof prizeListArr)
		}
	}
</script>
